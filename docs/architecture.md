...
Telegram-бот показывает профиль, тарифы, остаток времени, ключи и поддержку. YooKassa в текущей private beta принудительно работает в test mode. После подтверждённого provider-события control plane идемпотентно меняет подписку; неподтверждённый webhook сам по себе не выдаёт минуты.
